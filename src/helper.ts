...
interface HelperTransactionSummary {
    urgid: string;
    title: string;
    details: string;
...
const devices = data.devices.slice().sort((left, right) => left.name.localeCompare(right.name));
...
async function handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
    try {
        ...
        if (url.pathname.startsWith("/api/") && !hasToken(request)) {
        ...
        if (request.method === "POST" && url.pathname === "/api/approve") {
            ...
            const device = getDevice(String(body.pkey ?? ""));
            ...
            await client.approveTransaction(device, transactions, urgid, verificationCode);
            writeJson(response, 200, await buildSnapshot());
            return;
        }
        ...
        if (request.method === "POST" && url.pathname === "/api/deny") {
            ...
            await client.approveTransaction(device, transactions, "__deny_all__");
            writeJson(response, 200, await buildSnapshot());
            return;
        }
...
function hasToken(request: IncomingMessage): boolean {
    return request.headers["x-vsduo-token"] === token;
}

function getDevice(pkey: string): DuoDevice {
...
async function buildSnapshot(): Promise<HelperSnapshot> {
  ...
  <style>
    ...
    * {
    ...
    body {
      ...
      color: var(--text);
      background:
        radial-gradient(circle at top left, rgba(185, 99, 51, 0.18), transparent 28%),
    ...
    .card {
      ...
      border-radius: 24px;
      padding: 18px;
      box-shadow: 0 18px 50px rgba(51, 34, 12, 0.07);
    }
  ...
  <script>
    ...
    function render(snapshot) {
      ...
      if (!snapshot.devices.length) {
        devicesRoot.append(el("div", "card", "No Duo devices are available in this helper session."));
        return;
      }
      ...
      for (const device of snapshot.devices) {
        ...
        const right = el("div");
        if (device.totp) {
          right.append(el("div", "totp", "TOTP " + device.totp));
        }
        header.append(right);
        card.append(header);
        ...
        denyButton.addEventListener("click", async () => {
          try {
            ...
            const snapshot = await request("/api/deny", {
              method: "POST",
              body: JSON.stringify({ pkey: device.pkey }),
            });
            render(snapshot);
        ...
        if (device.error) {
          card.append(el("div", "error", device.error));
        }

        const list = el("div", "transaction-list");
        if (!device.transactions.length) {
          list.append(el("div", "empty", "No pending transactions right now."));
        }
        ...
        for (const transaction of device.transactions) {
          ...
          if (transaction.verificationDigits) {
            item.append(el("div", "transaction-details", "Requires " + transaction.verificationDigits + " verification digits."));
          }
          ...
          approveButton.addEventListener("click", async () => {
            try {
              ...
              const snapshot = await request("/api/approve", {
                method: "POST",
                body: JSON.stringify({
                  pkey: device.pkey,
                  urgid: transaction.urgid,
    ...
    refreshButton.addEventListener("click", async () => {
      setFlash("Refreshing Duo transactions...");
      await refresh();
    });
